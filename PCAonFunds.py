...
class FactorClusterer:
    ...
    def __init__(
        ...
        if not isinstance(factor_series, pd.Series):
            factor_series = pd.Series(factor_series)
        self.factor = factor_series.rename('PC1')
        if not isinstance(fund_returns, pd.DataFrame):
    ...
    def align_data(self):
        ...
        factor = self.factor[~self.factor.index.duplicated(keep='first')]
        df_all = pd.concat([funds, factor.rename('PC1')], axis=1, join='inner')
        self.aligned_funds  = df_all[funds.columns]
    ...
    def compute_exposures(self):
        ...
        if not isinstance(X, pd.DataFrame):
            X = pd.DataFrame(X, index=self.aligned_factor.index, columns=pd.Index(['const', 'PC1']))
        X = X.astype(float)
        for fund in self.aligned_funds.columns:
            y = pd.to_numeric(self.aligned_funds[fund], errors='coerce')
    ...
    def standardize(self):
        ...
        self.features = pd.DataFrame(Z, index=self.exposure_df.index, columns=pd.Index(['beta_z']))
...
if __name__ == "__main__":
    ...
    print(fund_table)
    fund_table.to_excel(r"C:\repos\factors\fund_table2.xlsx")
